...
export default function Round2() {
  ...
  const currentTeamName =
    currentTeamKey === 'team1' ? teams.team1 : teams.team2;
  ...
  const handleOption = (optIndex) => {
    if (optIndex === q.answer) {
      ...
      setFeedback(
        `Salah. Jawaban benar: ${String.fromCharCode(65 + q.answer)}.`
      );
    }
    ...
    setTimeout(() => {
      ...
      if (next >= questions.length) {
        // ronde selesai → update ke ronde 3
        setCurrentRound(3); // otomatis simpan ke localStorage
        navigate('/scoreboard');
      } else {
        setIndex(next);
        setFeedback('');
